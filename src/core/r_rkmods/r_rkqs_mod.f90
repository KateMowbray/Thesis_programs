MODULE M_rkqsR 
 ...
 SUBROUTINE RKQS (R,DRDT,U,DUDT, T,HTRY,MU,EPS,RSCAL,HDID,HNEXT,T1,T2, uflag)
...
! CHARACTER(LEN=35)			:: tempfile
 ...
 DO WHILE (.NOT.(Rlost)) 
   CALL RKCK(R,DRDT,U,DUDT,T,H,MU,RTEMP,UTEMP, RERR,T1,T2)

   ERRMAX=maxval(abs(RERR(:)/RSCAL(:)))/EPS
  !PRINT*, maxloc(abs(RERR)) 
