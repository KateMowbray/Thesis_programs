...
! km: only changed to add functionality for CMT2
...
! km: added generic bounds for the test case so that it runs properly
   ...
   zee=(/(-1.0),1.0/)
   print*, '----'
   PRINT*, '..evaluating particle array against test grid..'
  ...
  IF (Alphasteps == 2) THEN
     dalpha=0
  ELSE
     dalpha = (AlphaMax-Alphamin)/(Alphasteps - 2.0d0) !added by S.Oskoui
  ENDIF
  !km: denominator adjusted to Alphasteps-2 for correct stepsize
  ...
  DO WHILE (pos_no_x .LE. RSTEPS(1)-1)
   DO WHILE (pos_no_y .LE. RSTEPS(2)-1)
    1066 DO WHILE ((pos_no_z .LE. RSTEPS(3)-1).AND.(pn .LE. pnmax))
     DO pos_no_alpha = 1, AlphaSteps-1,1 !km: first and last values have 1 taken off
      DO pos_no_ekin = 0, EkinSteps-1,1 !km: same here to ensure alphamin and ekinmin are considered
	...
	maxwellEfirst = .FALSE.
        ELSE IF (EkinSteps == 1) THEN !added to avoid NaN if EkinSteps=1
        Ekin=EKinLow
        ELSE
        Ekin=EKinLow+(EKinHigh-EKinLow)*pos_no_ekin/(EkinSteps*1.0d0-1.0)
       !km: denominator adjusted to be Ekinsteps-1
       ENDIF
