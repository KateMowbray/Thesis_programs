...
! km: I've adjusted this to include the full convective derivative of db/dt for the curvature drift
...
!------------------------------------------------------------------------------!   
SUBROUTINE DERIVS (T, R, DRDT, U, DUDT,MU, T1, T2)
!+ requires fields module for dot and cross product functions to work.
...
! PRINT*, 'DER: GRADDRIFT', GRADDRIFT 
 ...
 DmodBDT=GRADBT-dot(B,DlittleBDT)
!km: This line has been moved up so the dlittleBdt is a partial derivative


!km: Need to add terms to the full t derivatives that come about due to U and VE

dVedT = dVedT + VEGRADVE + VPAR*dVEdS
dlittleBdT = dlittleBdT + VEGRADlittleB + VPAR*dlittleBdS
...
!km: these last two terms are sorted when adding the extra terms to dlittleBdT
