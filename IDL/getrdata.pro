...
IF n_elements(n_c) eq 0 THEN n_c=35 ;km: Was 29, changed for e4-e9

;files=FINDFILE(wkdir+'RV*.dat',count=count)
files=FILE_SEARCH(wkdir+'RV*.dat',count=count)
IF count eq 0 THEN BEGIN
 PRINT, "ERROR: no RV*.dat files found"
 return, 0
ENDIF
IF (ii lt 1L) or (ii gt count) THEN BEGIN
...
IF keyword_set(ke) or keyword_set(all) THEN BEGIN
 ...
 e3=transpose(A(15,*))
 ke=transpose(A(16,*))
 e4=transpose(A(29,*)) ; km: my changes are present for these few lines where we can keep track of energisation terms, these are detailed in r_rkdrive_mod
 e5=transpose(A(30,*))
 e6=transpose(A(31,*))
 e7=transpose(A(32,*))
 e8=transpose(A(33,*))
 e9=transpose(A(34,*))
 thedata=create_struct(thedata,{e2:e2,e3:e3,ke:ke,e4:e4,e5:e5,e6:e6,e7:e7,e8:e8,e9:e9})
ENDIF
